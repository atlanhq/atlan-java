...
tasks.withType<JavaCompile> {
    ...
    // Generated test files contain very long single-expression builder chains (~1200 chained calls)
    // that cause javac's recursive type-resolution to overflow its default stack. Fork javac with
    // a larger stack so the compiler can handle them.
    options.isFork = true
    options.forkOptions.memoryMaximumSize = "2g"
    options.forkOptions.jvmArgs = listOf("-Xss8m")
