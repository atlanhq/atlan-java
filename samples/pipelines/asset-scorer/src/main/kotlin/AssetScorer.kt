...
object AssetScorer : AbstractNumaflowHandler(Handler) {
    ...
    object Handler : AtlanEventHandler {
        ...
        /** {@inheritDoc}  */
        override fun validatePrerequisites(
            event: AtlanEvent,
            logger: Logger,
        ): Boolean {
        ...
        override fun calculateChanges(
            ...
            val score: Double =
                if (asset is GlossaryTerm) {
                    ...
                    val sCertificate =
                        when (asset.certificateStatus) {
                            CertificateStatus.DRAFT -> 15
                            CertificateStatus.VERIFIED -> 25
                            else -> 0
                        }
                    val sReadme =
                        if (AtlanEventHandler.hasReadme(asset)) {
                            when {
        ...
        override fun hasChanges(
            ...
            val scoreOriginal =
                if (original.customMetadataSets?.containsKey(CM_SCORING) == true) {
                    original.customMetadataSets[CM_SCORING]!!.attributes[CM_ATTR_COMPOSITE_SCORE] ?: -1
                } else {
                    -1
                }
            val scoreModified =
                if (modified.customMetadataSets?.containsKey(CM_SCORING) == true) {
