...
/**
 * Test that passing through a connection qualifiedName (i.e., from connection widget) does not override it.
 */
class ConnectionWidgetPassthroughTest : PackageTest("cwp") {
    override val logger = Utils.getLogger(this.javaClass.name)

    private val conn1 = makeUnique("c1")
    private val conn1Type = "custom-passthrough"
    private val epochForConnection = (System.currentTimeMillis() / 1000) - 1

    private val testFile = "passthrough-assets.csv"
            ...
            lines.forEach { line ->
                val revised =
                    line
                        ...
                        .replace("{EPOCH}", "$epochForConnection")
                        .replace("{CNAME}", conn1)
    ...
    fun connection1Created() {
        ...
        assertEquals("default/$conn1Type/$epochForConnection", c1.qualifiedName)
    }
