...
publishing {
    publications {
        create<MavenPublication>("mavenJavaPkgCfg") {
            ...
            artifactId = providers.gradleProperty("AIM_ARTIFACT_ID").get()
            version = providers.gradleProperty("VERSION_NAME").get()
            from(components["java"])
            pom {
                name.set(providers.gradleProperty("AIM_ARTIFACT_ID").get())
                description.set(providers.gradleProperty("AIM_DESCRIPTION").get())
