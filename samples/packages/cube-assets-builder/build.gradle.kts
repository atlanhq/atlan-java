...
publishing {
    publications {
        create<MavenPublication>("mavenJavaPkgCfg") {
            ...
            artifactId = providers.gradleProperty("CAB_ARTIFACT_ID").get()
            version = providers.gradleProperty("VERSION_NAME").get()
            from(components["java"])
            pom {
                name.set(providers.gradleProperty("CAB_ARTIFACT_ID").get())
                description.set(providers.gradleProperty("CAB_DESCRIPTION").get())
