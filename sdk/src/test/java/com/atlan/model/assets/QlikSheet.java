...
public class QlikSheet extends Asset implements IQlikSheet, IQlik, IBI, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Reference to a QlikSheet by GUID. Use this to create a relationship to this QlikSheet,
    ...
    public static QlikSheet refByGuid(String guid, Reference.SaveSemantic semantic) {
        return QlikSheet._internal().guid(guid).semantic(semantic).build();
    }

    /**
     * Reference to a QlikSheet by qualifiedName. Use this to create a relationship to this QlikSheet,
     * where the relationship should be replaced.
     *
    ...
    /**
     * Retrieves a QlikSheet by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the QlikSheet to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the QlikSheet, including any relationships
     * @return the requested QlikSheet, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the QlikSheet does not exist or the provided GUID is not a QlikSheet
     */
    @JsonIgnore
     ...
     * @param id of the QlikSheet to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the QlikSheet, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the QlikSheet
     * @return the requested QlikSheet, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the QlikSheet does not exist or the provided GUID is not a QlikSheet
     */
    @JsonIgnore
    public static QlikSheet get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = QlikSheet.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof QlikSheet) {
    ...
    public static QlikSheet updateAnnouncement(
            ...
            throws AtlanException {
        return (QlikSheet) Asset.updateAnnouncement(client, _internal(), TYPE_NAME, qualifiedName, type, title, message);
    }
     ...
     * @throws AtlanException on any API problems
     */
    public static QlikSheet replaceTerms(AtlanClient client, String qualifiedName, String name, List<IGlossaryTerm> terms)
            throws AtlanException {
