...
public class KafkaConsumerGroup extends Asset implements IKafkaConsumerGroup, IKafka, IEventStore, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a KafkaConsumerGroup by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the KafkaConsumerGroup to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the KafkaConsumerGroup, including any relationships
     * @return the requested KafkaConsumerGroup, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the KafkaConsumerGroup does not exist or the provided GUID is not a KafkaConsumerGroup
     */
    @JsonIgnore
     ...
     * @param id of the KafkaConsumerGroup to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the KafkaConsumerGroup, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the KafkaConsumerGroup
     * @return the requested KafkaConsumerGroup, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the KafkaConsumerGroup does not exist or the provided GUID is not a KafkaConsumerGroup
     */
    @JsonIgnore
    public static KafkaConsumerGroup get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = KafkaConsumerGroup.select(client)
                    .where(KafkaConsumerGroup.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof KafkaConsumerGroup) {
            ...
            Optional<Asset> asset = KafkaConsumerGroup.select(client)
                    .where(KafkaConsumerGroup.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .includeRelationshipAttributes(true)
                    .pageSize(1)
...
/**
    ...
    public abstract static class KafkaConsumerGroupBuilder<C extends KafkaConsumerGroup, B extends KafkaConsumerGroupBuilder<C, B>>
     ...
     * @throws AtlanException on any API problems
     */
    public static KafkaConsumerGroup replaceTerms(AtlanClient client, String qualifiedName, String name, List<IGlossaryTerm> terms)
            throws AtlanException {
     ...
     */
    @Deprecated
    public static KafkaConsumerGroup appendAtlanTags(AtlanClient client, String qualifiedName, List<String> atlanTagNames)
            throws AtlanException {
