...
public class TypeRegistryEnumTest {

    private static final TypeRegistryEnum full = TypeRegistryEnum._internal()
            .guid("guid")
            ...
            .typeRegistryValue(TypeRegistryValidValue.builder()
                    .typeRegistryValue("String0")
                    .typeRegistryOrdinal(123456789L)
                    .typeRegistryDescription("String0")
                    .build())
            .typeRegistryValue(TypeRegistryValidValue.builder()
                    .typeRegistryValue("String1")
                    .typeRegistryOrdinal(987654321L)
                    .typeRegistryDescription("String1")
                    .build())
            .build();

    private static final int hash = full.hashCode();
    private static TypeRegistryEnum frodo;
    private static String serialized;

    @Test(groups = {"TypeRegistryEnum.builderEquivalency"})
    void builderEquivalency() {
        assertEquals(full.toBuilder().build(), full);
    }

    @Test(
            groups = {"TypeRegistryEnum.serialize"},
            dependsOnGroups = {"TypeRegistryEnum.builderEquivalency"})
    void serialization() {
        assertNotNull(full);
    ...
    @Test(
            groups = {"TypeRegistryEnum.deserialize"},
            dependsOnGroups = {"TypeRegistryEnum.serialize"})
    void deserialization() throws IOException {
        assertNotNull(serialized);
        frodo = MockAtlanTenant.client.readValue(serialized, TypeRegistryEnum.class);
        assertNotNull(frodo);
    }

    @Test(
            groups = {"TypeRegistryEnum.equivalency"},
            dependsOnGroups = {"TypeRegistryEnum.serialize", "TypeRegistryEnum.deserialize"})
