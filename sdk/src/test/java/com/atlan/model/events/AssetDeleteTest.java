...
public class AssetDeleteTest {
    ...
    @Test
    void serdeCycleAssetDeleteEvent() throws IOException {
        assertNotNull(full, "Unable to build sample instance of AssetDeleteEvent,");
        final int hash = full.hashCode();
        // Builder equivalency
        assertEquals(
                full.toBuilder().build(),
                full,
                "Unable to converting AssetDeleteEvent via builder back to its original state,");
        // Serialization
        final String serialized = full.toJson(MockTenant.client);
        assertNotNull(serialized, "Unable to serialize sample instance of AssetDeleteEvent,");
        assertEquals(full.hashCode(), hash, "Serialization mutated the original value,");
        // Deserialization
        final AtlanEvent frodo = MockTenant.client.readValue(serialized, AtlanEvent.class);
        assertNotNull(frodo, "Unable to reverse-read serialized value back into an instance of AssetDeleteEvent,");
