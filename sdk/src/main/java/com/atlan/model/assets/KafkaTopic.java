...
public class KafkaTopic extends Asset implements IKafkaTopic, IKafka, IEventStore, ICatalog, IAsset, IReferenceable {
     ...
     */
    @JsonIgnore
    public static KafkaTopic get(AtlanClient client, String id, boolean includeAllRelationships) throws AtlanException {
        if (id == null) {
    ...
    /**
     * Retrieves a KafkaTopic by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the KafkaTopic to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the KafkaTopic, including any relationships
     * @return the requested KafkaTopic, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the KafkaTopic does not exist or the provided GUID is not a KafkaTopic
     */
    @JsonIgnore
     ...
     * @param id of the KafkaTopic to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the KafkaTopic, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the KafkaTopic
     * @return the requested KafkaTopic, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the KafkaTopic does not exist or the provided GUID is not a KafkaTopic
     */
    @JsonIgnore
    public static KafkaTopic get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = KafkaTopic.select(client)
                    .where(KafkaTopic.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof KafkaTopic) {
                return (KafkaTopic) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
            Optional<Asset> asset = KafkaTopic.select(client)
                    .where(KafkaTopic.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .includeRelationshipAttributes(true)
                    .pageSize(1)
