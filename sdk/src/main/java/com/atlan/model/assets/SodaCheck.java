...
public class SodaCheck extends Asset implements ISodaCheck, ISoda, IDataQuality, ICatalog, IAsset, IReferenceable {
     ...
     */
    @JsonIgnore
    public static SodaCheck get(AtlanClient client, String id, boolean includeAllRelationships) throws AtlanException {
        if (id == null) {
    ...
    /**
     * Retrieves a SodaCheck by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the SodaCheck to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SodaCheck, including any relationships
     * @return the requested SodaCheck, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SodaCheck does not exist or the provided GUID is not a SodaCheck
     */
    @JsonIgnore
     ...
     * @param id of the SodaCheck to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SodaCheck, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the SodaCheck
     * @return the requested SodaCheck, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SodaCheck does not exist or the provided GUID is not a SodaCheck
     */
    @JsonIgnore
    public static SodaCheck get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = SodaCheck.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof SodaCheck) {
                return (SodaCheck) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
