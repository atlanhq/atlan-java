...
public class SchemaRegistrySubject extends Asset
    ...
    /**
     * Retrieves a SchemaRegistrySubject by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the SchemaRegistrySubject to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SchemaRegistrySubject, including any relationships
     * @return the requested SchemaRegistrySubject, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SchemaRegistrySubject does not exist or the provided GUID is not a SchemaRegistrySubject
     */
    @JsonIgnore
     ...
     * @param id of the SchemaRegistrySubject to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SchemaRegistrySubject, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the SchemaRegistrySubject
     * @return the requested SchemaRegistrySubject, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SchemaRegistrySubject does not exist or the provided GUID is not a SchemaRegistrySubject
     */
    @JsonIgnore
    public static SchemaRegistrySubject get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = SchemaRegistrySubject.select(client)
                    .where(SchemaRegistrySubject.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof SchemaRegistrySubject) {
                return (SchemaRegistrySubject) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
            Optional<Asset> asset = SchemaRegistrySubject.select(client)
                    .where(SchemaRegistrySubject.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .pageSize(1)
                    .stream()
