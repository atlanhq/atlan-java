...
public class TableauSite extends Asset implements ITableauSite, ITableau, IBI, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a TableauSite by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the TableauSite to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the TableauSite, including any relationships
     * @return the requested TableauSite, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the TableauSite does not exist or the provided GUID is not a TableauSite
     */
    @JsonIgnore
     ...
     * @param id of the TableauSite to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the TableauSite, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the TableauSite
     * @return the requested TableauSite, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the TableauSite does not exist or the provided GUID is not a TableauSite
     */
    @JsonIgnore
    public static TableauSite get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = TableauSite.select(client)
                    .where(TableauSite.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof TableauSite) {
                return (TableauSite) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
            Optional<Asset> asset = TableauSite.select(client)
                    .where(TableauSite.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .includeRelationshipAttributes(true)
                    .pageSize(1)
    ...
    public abstract static class TableauSiteBuilder<C extends TableauSite, B extends TableauSiteBuilder<C, B>>
