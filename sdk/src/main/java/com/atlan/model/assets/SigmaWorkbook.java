...
public class SigmaWorkbook extends Asset implements ISigmaWorkbook, ISigma, IBI, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a SigmaWorkbook by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the SigmaWorkbook to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SigmaWorkbook, including any relationships
     * @return the requested SigmaWorkbook, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SigmaWorkbook does not exist or the provided GUID is not a SigmaWorkbook
     */
    @JsonIgnore
     ...
     * @param id of the SigmaWorkbook to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SigmaWorkbook, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the SigmaWorkbook
     * @return the requested SigmaWorkbook, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SigmaWorkbook does not exist or the provided GUID is not a SigmaWorkbook
     */
    @JsonIgnore
    public static SigmaWorkbook get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = SigmaWorkbook.select(client)
                    .where(SigmaWorkbook.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof SigmaWorkbook) {
            ...
            Optional<Asset> asset = SigmaWorkbook.select(client)
                    .where(SigmaWorkbook.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .pageSize(1)
                    .stream()
