...
public class MatillionProject extends Asset implements IMatillionProject, IMatillion, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a MatillionProject by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the MatillionProject to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MatillionProject, including any relationships
     * @return the requested MatillionProject, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MatillionProject does not exist or the provided GUID is not a MatillionProject
     */
    @JsonIgnore
     ...
     * @param id of the MatillionProject to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MatillionProject, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the MatillionProject
     * @return the requested MatillionProject, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MatillionProject does not exist or the provided GUID is not a MatillionProject
     */
    @JsonIgnore
    public static MatillionProject get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = MatillionProject.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof MatillionProject) {
                return (MatillionProject) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
