...
public class APIQuery extends Asset implements IAPIQuery, IAPI, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a APIQuery by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the APIQuery to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the APIQuery, including any relationships
     * @return the requested APIQuery, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the APIQuery does not exist or the provided GUID is not a APIQuery
     */
    @JsonIgnore
     ...
     * @param id of the APIQuery to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the APIQuery, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the APIQuery
     * @return the requested APIQuery, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the APIQuery does not exist or the provided GUID is not a APIQuery
     */
    @JsonIgnore
    public static APIQuery get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = APIQuery.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof APIQuery) {
