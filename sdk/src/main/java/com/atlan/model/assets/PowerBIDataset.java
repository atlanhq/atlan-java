...
public class PowerBIDataset extends Asset implements IPowerBIDataset, IPowerBI, IBI, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a PowerBIDataset by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the PowerBIDataset to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the PowerBIDataset, including any relationships
     * @return the requested PowerBIDataset, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the PowerBIDataset does not exist or the provided GUID is not a PowerBIDataset
     */
    @JsonIgnore
     ...
     * @param id of the PowerBIDataset to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the PowerBIDataset, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the PowerBIDataset
     * @return the requested PowerBIDataset, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the PowerBIDataset does not exist or the provided GUID is not a PowerBIDataset
     */
    @JsonIgnore
    public static PowerBIDataset get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = PowerBIDataset.select(client)
                    .where(PowerBIDataset.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof PowerBIDataset) {
                return (PowerBIDataset) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
            Optional<Asset> asset = PowerBIDataset.select(client)
                    .where(PowerBIDataset.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .pageSize(1)
                    .stream()
