...
public class SparkJob extends Asset implements ISparkJob, ISpark, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a SparkJob by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the SparkJob to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SparkJob, including any relationships
     * @return the requested SparkJob, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SparkJob does not exist or the provided GUID is not a SparkJob
     */
    @JsonIgnore
     ...
     * @param id of the SparkJob to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SparkJob, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the SparkJob
     * @return the requested SparkJob, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SparkJob does not exist or the provided GUID is not a SparkJob
     */
    @JsonIgnore
    public static SparkJob get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = SparkJob.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof SparkJob) {
