...
public class MicroStrategyCube extends Asset
    ...
    /**
     * Retrieves a MicroStrategyCube by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the MicroStrategyCube to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MicroStrategyCube, including any relationships
     * @return the requested MicroStrategyCube, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MicroStrategyCube does not exist or the provided GUID is not a MicroStrategyCube
     */
    @JsonIgnore
     ...
     * @param id of the MicroStrategyCube to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MicroStrategyCube, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the MicroStrategyCube
     * @return the requested MicroStrategyCube, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MicroStrategyCube does not exist or the provided GUID is not a MicroStrategyCube
     */
    @JsonIgnore
    public static MicroStrategyCube get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = MicroStrategyCube.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof MicroStrategyCube) {
                return (MicroStrategyCube) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
