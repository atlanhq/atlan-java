...
public class SisenseFolder extends Asset implements ISisenseFolder, ISisense, IBI, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a SisenseFolder by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the SisenseFolder to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SisenseFolder, including any relationships
     * @return the requested SisenseFolder, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SisenseFolder does not exist or the provided GUID is not a SisenseFolder
     */
    @JsonIgnore
     ...
     * @param id of the SisenseFolder to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SisenseFolder, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the SisenseFolder
     * @return the requested SisenseFolder, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SisenseFolder does not exist or the provided GUID is not a SisenseFolder
     */
    @JsonIgnore
    public static SisenseFolder get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = SisenseFolder.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof SisenseFolder) {
