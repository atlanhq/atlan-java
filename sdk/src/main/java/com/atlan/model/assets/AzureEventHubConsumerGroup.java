...
public class AzureEventHubConsumerGroup extends Asset
    ...
    /**
     * Retrieves a AzureEventHubConsumerGroup by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the AzureEventHubConsumerGroup to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the AzureEventHubConsumerGroup, including any relationships
     * @return the requested AzureEventHubConsumerGroup, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the AzureEventHubConsumerGroup does not exist or the provided GUID is not a AzureEventHubConsumerGroup
     */
    @JsonIgnore
     ...
     * @param id of the AzureEventHubConsumerGroup to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the AzureEventHubConsumerGroup, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the AzureEventHubConsumerGroup
     * @return the requested AzureEventHubConsumerGroup, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the AzureEventHubConsumerGroup does not exist or the provided GUID is not a AzureEventHubConsumerGroup
     */
    @JsonIgnore
    public static AzureEventHubConsumerGroup get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = AzureEventHubConsumerGroup.select(client)
                    .where(AzureEventHubConsumerGroup.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof AzureEventHubConsumerGroup) {
                return (AzureEventHubConsumerGroup) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
            Optional<Asset> asset = AzureEventHubConsumerGroup.select(client)
                    .where(AzureEventHubConsumerGroup.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .pageSize(1)
                    .stream()
