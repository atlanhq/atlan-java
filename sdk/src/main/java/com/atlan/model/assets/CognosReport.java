...
public class CognosReport extends Asset implements ICognosReport, ICognos, IBI, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a CognosReport by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the CognosReport to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the CognosReport, including any relationships
     * @return the requested CognosReport, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the CognosReport does not exist or the provided GUID is not a CognosReport
     */
    @JsonIgnore
     ...
     * @param id of the CognosReport to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the CognosReport, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the CognosReport
     * @return the requested CognosReport, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the CognosReport does not exist or the provided GUID is not a CognosReport
     */
    @JsonIgnore
    public static CognosReport get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = CognosReport.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof CognosReport) {
                return (CognosReport) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
