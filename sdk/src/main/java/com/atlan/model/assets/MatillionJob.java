...
public class MatillionJob extends Asset implements IMatillionJob, IMatillion, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Reference to a MatillionJob by GUID. Use this to create a relationship to this MatillionJob,
    ...
    public static MatillionJob refByGuid(String guid, Reference.SaveSemantic semantic) {
        return MatillionJob._internal().guid(guid).semantic(semantic).build();
    }

    /**
     * Reference to a MatillionJob by qualifiedName. Use this to create a relationship to this MatillionJob,
     * where the relationship should be replaced.
     *
    ...
    /**
     * Retrieves a MatillionJob by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the MatillionJob to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MatillionJob, including any relationships
     * @return the requested MatillionJob, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MatillionJob does not exist or the provided GUID is not a MatillionJob
     */
    @JsonIgnore
     ...
     * @param id of the MatillionJob to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MatillionJob, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the MatillionJob
     * @return the requested MatillionJob, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MatillionJob does not exist or the provided GUID is not a MatillionJob
     */
    @JsonIgnore
    public static MatillionJob get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = MatillionJob.select(client)
                    .where(MatillionJob.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof MatillionJob) {
            ...
            Optional<Asset> asset = MatillionJob.select(client)
                    .where(MatillionJob.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .includeRelationshipAttributes(true)
                    .pageSize(1)
    ...
    /**
     * Builds the minimal object necessary to apply an update to a MatillionJob,
     * from a potentially more-complete MatillionJob object.
     *
     * @return the minimal object necessary to update the MatillionJob, as a builder
     * @throws InvalidRequestException if any of the minimal set of required fields for a MatillionJob are not present in the initial object
     */
    @Override
