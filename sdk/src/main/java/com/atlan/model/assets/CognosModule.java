...
public class CognosModule extends Asset implements ICognosModule, ICognos, IBI, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a CognosModule by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the CognosModule to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the CognosModule, including any relationships
     * @return the requested CognosModule, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the CognosModule does not exist or the provided GUID is not a CognosModule
     */
    @JsonIgnore
     ...
     * @param id of the CognosModule to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the CognosModule, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the CognosModule
     * @return the requested CognosModule, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the CognosModule does not exist or the provided GUID is not a CognosModule
     */
    @JsonIgnore
    public static CognosModule get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = CognosModule.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof CognosModule) {
                return (CognosModule) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
