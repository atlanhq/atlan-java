...
public class SnowflakeTag extends Asset implements ISnowflakeTag, ITag, ISQL, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a SnowflakeTag by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the SnowflakeTag to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SnowflakeTag, including any relationships
     * @return the requested SnowflakeTag, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SnowflakeTag does not exist or the provided GUID is not a SnowflakeTag
     */
    @JsonIgnore
     ...
     * @param id of the SnowflakeTag to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SnowflakeTag, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the SnowflakeTag
     * @return the requested SnowflakeTag, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SnowflakeTag does not exist or the provided GUID is not a SnowflakeTag
     */
    @JsonIgnore
    public static SnowflakeTag get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = SnowflakeTag.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof SnowflakeTag) {
                return (SnowflakeTag) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
