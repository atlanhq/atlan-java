...
public class BusinessPolicyException extends Asset implements IBusinessPolicyException, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a BusinessPolicyException by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the BusinessPolicyException to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the BusinessPolicyException, including any relationships
     * @return the requested BusinessPolicyException, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the BusinessPolicyException does not exist or the provided GUID is not a BusinessPolicyException
     */
    @JsonIgnore
     ...
     * @param id of the BusinessPolicyException to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the BusinessPolicyException, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the BusinessPolicyException
     * @return the requested BusinessPolicyException, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the BusinessPolicyException does not exist or the provided GUID is not a BusinessPolicyException
     */
    @JsonIgnore
    public static BusinessPolicyException get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = BusinessPolicyException.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof BusinessPolicyException) {
