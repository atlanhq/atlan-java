...
public class AdfDataset extends Asset implements IAdfDataset, IADF, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a AdfDataset by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the AdfDataset to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the AdfDataset, including any relationships
     * @return the requested AdfDataset, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the AdfDataset does not exist or the provided GUID is not a AdfDataset
     */
    @JsonIgnore
     ...
     * @param id of the AdfDataset to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the AdfDataset, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the AdfDataset
     * @return the requested AdfDataset, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the AdfDataset does not exist or the provided GUID is not a AdfDataset
     */
    @JsonIgnore
    public static AdfDataset get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = AdfDataset.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof AdfDataset) {
