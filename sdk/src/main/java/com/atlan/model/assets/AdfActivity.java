...
public class AdfActivity extends Asset implements IAdfActivity, IADF, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a AdfActivity by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the AdfActivity to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the AdfActivity, including any relationships
     * @return the requested AdfActivity, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the AdfActivity does not exist or the provided GUID is not a AdfActivity
     */
    @JsonIgnore
     ...
     * @param id of the AdfActivity to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the AdfActivity, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the AdfActivity
     * @return the requested AdfActivity, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the AdfActivity does not exist or the provided GUID is not a AdfActivity
     */
    @JsonIgnore
    public static AdfActivity get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = AdfActivity.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof AdfActivity) {
