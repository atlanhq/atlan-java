...
public class DbtMetric extends Asset implements IDbtMetric, IMetric, IDbt, IDataQuality, ICatalog, IAsset, IReferenceable {
    ...
    @Attribute
    @Singular
    @JsonProperty("metric_metricDimensionColumns")
    SortedSet<IColumn> metricDimensionColumns;
    ...
    @Attribute
    @JsonProperty("metric_metricTimestampColumn")
    IColumn metricTimestampColumn;
    ...
    /**
     * Retrieves a DbtMetric by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the DbtMetric to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the DbtMetric, including any relationships
     * @return the requested DbtMetric, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the DbtMetric does not exist or the provided GUID is not a DbtMetric
     */
    @JsonIgnore
     ...
     * @param id of the DbtMetric to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the DbtMetric, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the DbtMetric
     * @return the requested DbtMetric, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the DbtMetric does not exist or the provided GUID is not a DbtMetric
     */
    @JsonIgnore
    public static DbtMetric get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = DbtMetric.select(client)
                    .where(DbtMetric.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof DbtMetric) {
            ...
            Optional<Asset> asset = DbtMetric.select(client)
                    .where(DbtMetric.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .includeRelationshipAttributes(true)
                    .pageSize(1)
