...
public class TableauCalculatedField extends Asset
    ...
    /**
     * Retrieves a TableauCalculatedField by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the TableauCalculatedField to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the TableauCalculatedField, including any relationships
     * @return the requested TableauCalculatedField, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the TableauCalculatedField does not exist or the provided GUID is not a TableauCalculatedField
     */
    @JsonIgnore
     ...
     * @param id of the TableauCalculatedField to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the TableauCalculatedField, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the TableauCalculatedField
     * @return the requested TableauCalculatedField, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the TableauCalculatedField does not exist or the provided GUID is not a TableauCalculatedField
     */
    @JsonIgnore
    public static TableauCalculatedField get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = TableauCalculatedField.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof TableauCalculatedField) {
                return (TableauCalculatedField) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
