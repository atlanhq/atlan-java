...
public class DynamoDBGlobalSecondaryIndex extends Asset
    ...
    /**
     * Retrieves a DynamoDBGlobalSecondaryIndex by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the DynamoDBGlobalSecondaryIndex to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the DynamoDBGlobalSecondaryIndex, including any relationships
     * @return the requested DynamoDBGlobalSecondaryIndex, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the DynamoDBGlobalSecondaryIndex does not exist or the provided GUID is not a DynamoDBGlobalSecondaryIndex
     */
    @JsonIgnore
     ...
     * @param id of the DynamoDBGlobalSecondaryIndex to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the DynamoDBGlobalSecondaryIndex, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the DynamoDBGlobalSecondaryIndex
     * @return the requested DynamoDBGlobalSecondaryIndex, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the DynamoDBGlobalSecondaryIndex does not exist or the provided GUID is not a DynamoDBGlobalSecondaryIndex
     */
    @JsonIgnore
    public static DynamoDBGlobalSecondaryIndex get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = DynamoDBGlobalSecondaryIndex.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof DynamoDBGlobalSecondaryIndex) {
                return (DynamoDBGlobalSecondaryIndex) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
