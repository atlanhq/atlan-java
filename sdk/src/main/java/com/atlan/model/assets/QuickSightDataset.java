...
public class QuickSightDataset extends Asset
    ...
    /**
     * Retrieves a QuickSightDataset by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the QuickSightDataset to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the QuickSightDataset, including any relationships
     * @return the requested QuickSightDataset, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the QuickSightDataset does not exist or the provided GUID is not a QuickSightDataset
     */
    @JsonIgnore
     ...
     * @param id of the QuickSightDataset to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the QuickSightDataset, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the QuickSightDataset
     * @return the requested QuickSightDataset, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the QuickSightDataset does not exist or the provided GUID is not a QuickSightDataset
     */
    @JsonIgnore
    public static QuickSightDataset get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = QuickSightDataset.select(client)
                    .where(QuickSightDataset.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof QuickSightDataset) {
                return (QuickSightDataset) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
            Optional<Asset> asset = QuickSightDataset.select(client)
                    .where(QuickSightDataset.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .includeRelationshipAttributes(true)
                    .pageSize(1)
    ...
    /**
     * Builds the minimal object necessary to apply an update to a QuickSightDataset,
     * from a potentially more-complete QuickSightDataset object.
     *
     * @return the minimal object necessary to update the QuickSightDataset, as a builder
     * @throws InvalidRequestException if any of the minimal set of required fields for a QuickSightDataset are not present in the initial object
     */
    @Override
