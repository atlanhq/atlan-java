...
public class DbtProcess extends Asset implements IDbtProcess, IDbt, ILineageProcess, ICatalog, IAsset, IReferenceable {
     ...
     */
    @JsonIgnore
    public static DbtProcess get(AtlanClient client, String id, boolean includeAllRelationships) throws AtlanException {
        if (id == null) {
    ...
    /**
     * Retrieves a DbtProcess by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the DbtProcess to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the DbtProcess, including any relationships
     * @return the requested DbtProcess, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the DbtProcess does not exist or the provided GUID is not a DbtProcess
     */
    @JsonIgnore
     ...
     * @param id of the DbtProcess to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the DbtProcess, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the DbtProcess
     * @return the requested DbtProcess, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the DbtProcess does not exist or the provided GUID is not a DbtProcess
     */
    @JsonIgnore
    public static DbtProcess get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = DbtProcess.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof DbtProcess) {
                return (DbtProcess) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
