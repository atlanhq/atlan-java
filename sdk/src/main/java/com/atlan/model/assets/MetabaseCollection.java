...
public class MetabaseCollection extends Asset
    ...
    /**
     * Retrieves a MetabaseCollection by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the MetabaseCollection to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MetabaseCollection, including any relationships
     * @return the requested MetabaseCollection, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MetabaseCollection does not exist or the provided GUID is not a MetabaseCollection
     */
    @JsonIgnore
     ...
     * @param id of the MetabaseCollection to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MetabaseCollection, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the MetabaseCollection
     * @return the requested MetabaseCollection, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MetabaseCollection does not exist or the provided GUID is not a MetabaseCollection
     */
    @JsonIgnore
    public static MetabaseCollection get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = MetabaseCollection.select(client)
                    .where(MetabaseCollection.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof MetabaseCollection) {
            ...
            Optional<Asset> asset = MetabaseCollection.select(client)
                    .where(MetabaseCollection.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .includeRelationshipAttributes(true)
                    .pageSize(1)
