...
public class TableauProject extends Asset implements ITableauProject, ITableau, IBI, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a TableauProject by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the TableauProject to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the TableauProject, including any relationships
     * @return the requested TableauProject, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the TableauProject does not exist or the provided GUID is not a TableauProject
     */
    @JsonIgnore
     ...
     * @param id of the TableauProject to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the TableauProject, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the TableauProject
     * @return the requested TableauProject, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the TableauProject does not exist or the provided GUID is not a TableauProject
     */
    @JsonIgnore
    public static TableauProject get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = TableauProject.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof TableauProject) {
                return (TableauProject) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
