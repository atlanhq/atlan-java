...
public class DataDomain extends Asset implements IDataDomain, IDataMesh, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a DataDomain by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the DataDomain to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the DataDomain, including any relationships
     * @return the requested DataDomain, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the DataDomain does not exist or the provided GUID is not a DataDomain
     */
    @JsonIgnore
     ...
     * @param id of the DataDomain to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the DataDomain, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the DataDomain
     * @return the requested DataDomain, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the DataDomain does not exist or the provided GUID is not a DataDomain
     */
    @JsonIgnore
    public static DataDomain get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = DataDomain.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof DataDomain) {
