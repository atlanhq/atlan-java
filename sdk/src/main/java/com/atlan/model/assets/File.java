...
public class File extends Asset implements IFile, IResource, ICatalog, IAsset, IReferenceable {
     ...
     */
    @JsonIgnore
    public static File get(AtlanClient client, String id, boolean includeAllRelationships) throws AtlanException {
        if (id == null) {
    ...
    /**
     * Retrieves a File by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the File to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the File, including any relationships
     * @return the requested File, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the File does not exist or the provided GUID is not a File
     */
    @JsonIgnore
    public static File get(AtlanClient client, String id, Collection<AtlanField> attributes) throws AtlanException {
     ...
     * @param id of the File to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the File, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the File
     * @return the requested File, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the File does not exist or the provided GUID is not a File
     */
    @JsonIgnore
    public static File get(
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = File.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof File) {
                return (File) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
