...
public class SnowflakePipe extends Asset implements ISnowflakePipe, ISQL, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a SnowflakePipe by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the SnowflakePipe to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SnowflakePipe, including any relationships
     * @return the requested SnowflakePipe, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SnowflakePipe does not exist or the provided GUID is not a SnowflakePipe
     */
    @JsonIgnore
     ...
     * @param id of the SnowflakePipe to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SnowflakePipe, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the SnowflakePipe
     * @return the requested SnowflakePipe, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SnowflakePipe does not exist or the provided GUID is not a SnowflakePipe
     */
    @JsonIgnore
    public static SnowflakePipe get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = SnowflakePipe.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof SnowflakePipe) {
                return (SnowflakePipe) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
