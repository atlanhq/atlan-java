...
public class ReadmeTemplate extends Asset implements IReadmeTemplate, IResource, ICatalog, IAsset, IReferenceable {
     ...
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the ReadmeTemplate to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the ReadmeTemplate, including any relationships
     * @return the requested ReadmeTemplate, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the ReadmeTemplate does not exist or the provided GUID is not a ReadmeTemplate
     */
    @JsonIgnore
     ...
     * @param id of the ReadmeTemplate to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the ReadmeTemplate, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the ReadmeTemplate
     * @return the requested ReadmeTemplate, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the ReadmeTemplate does not exist or the provided GUID is not a ReadmeTemplate
     */
    @JsonIgnore
    public static ReadmeTemplate get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = ReadmeTemplate.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof ReadmeTemplate) {
                return (ReadmeTemplate) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
