...
public class DynamoDBLocalSecondaryIndex extends Asset
    ...
    /**
     * Retrieves a DynamoDBLocalSecondaryIndex by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the DynamoDBLocalSecondaryIndex to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the DynamoDBLocalSecondaryIndex, including any relationships
     * @return the requested DynamoDBLocalSecondaryIndex, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the DynamoDBLocalSecondaryIndex does not exist or the provided GUID is not a DynamoDBLocalSecondaryIndex
     */
    @JsonIgnore
     ...
     * @param id of the DynamoDBLocalSecondaryIndex to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the DynamoDBLocalSecondaryIndex, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the DynamoDBLocalSecondaryIndex
     * @return the requested DynamoDBLocalSecondaryIndex, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the DynamoDBLocalSecondaryIndex does not exist or the provided GUID is not a DynamoDBLocalSecondaryIndex
     */
    @JsonIgnore
    public static DynamoDBLocalSecondaryIndex get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = DynamoDBLocalSecondaryIndex.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof DynamoDBLocalSecondaryIndex) {
                return (DynamoDBLocalSecondaryIndex) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
