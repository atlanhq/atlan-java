...
public class MicroStrategyFact extends Asset
    ...
    /**
     * Retrieves a MicroStrategyFact by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the MicroStrategyFact to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MicroStrategyFact, including any relationships
     * @return the requested MicroStrategyFact, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MicroStrategyFact does not exist or the provided GUID is not a MicroStrategyFact
     */
    @JsonIgnore
     ...
     * @param id of the MicroStrategyFact to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MicroStrategyFact, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the MicroStrategyFact
     * @return the requested MicroStrategyFact, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MicroStrategyFact does not exist or the provided GUID is not a MicroStrategyFact
     */
    @JsonIgnore
    public static MicroStrategyFact get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = MicroStrategyFact.select(client)
                    .where(MicroStrategyFact.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof MicroStrategyFact) {
                return (MicroStrategyFact) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
            Optional<Asset> asset = MicroStrategyFact.select(client)
                    .where(MicroStrategyFact.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .pageSize(1)
                    .stream()
