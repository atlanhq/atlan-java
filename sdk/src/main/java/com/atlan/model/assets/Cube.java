...
public class Cube extends Asset implements ICube, IMultiDimensionalDataset, ICatalog, IAsset, IReferenceable {
     ...
     */
    @JsonIgnore
    public static Cube get(AtlanClient client, String id, boolean includeAllRelationships) throws AtlanException {
        if (id == null) {
    ...
    /**
     * Retrieves a Cube by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the Cube to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the Cube, including any relationships
     * @return the requested Cube, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the Cube does not exist or the provided GUID is not a Cube
     */
    @JsonIgnore
    public static Cube get(AtlanClient client, String id, Collection<AtlanField> attributes) throws AtlanException {
     ...
     * @param id of the Cube to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the Cube, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the Cube
     * @return the requested Cube, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the Cube does not exist or the provided GUID is not a Cube
     */
    @JsonIgnore
    public static Cube get(
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = Cube.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof Cube) {
                return (Cube) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
    ...
    /**
     * Builds the minimal object necessary to apply an update to a Cube,
     * from a potentially more-complete Cube object.
     *
     * @return the minimal object necessary to update the Cube, as a builder
     * @throws InvalidRequestException if any of the minimal set of required fields for a Cube are not present in the initial object
     */
    @Override
