...
public class Folder extends Asset implements IFolder, INamespace, IAsset, IReferenceable {
     ...
     */
    @JsonIgnore
    public static Folder get(AtlanClient client, String id, boolean includeAllRelationships) throws AtlanException {
        if (id == null) {
    ...
    /**
     * Retrieves a Folder by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the Folder to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the Folder, including any relationships
     * @return the requested Folder, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the Folder does not exist or the provided GUID is not a Folder
     */
    @JsonIgnore
    public static Folder get(AtlanClient client, String id, Collection<AtlanField> attributes) throws AtlanException {
     ...
     * @param id of the Folder to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the Folder, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the Folder
     * @return the requested Folder, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the Folder does not exist or the provided GUID is not a Folder
     */
    @JsonIgnore
    public static Folder get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = Folder.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof Folder) {
                return (Folder) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
     ...
     * @throws AtlanException on any API problems
     */
    public static Folder removeUserDescription(AtlanClient client, String qualifiedName, String name)
            throws AtlanException {
     ...
     */
    @Deprecated
    public static Folder appendTerms(AtlanClient client, String qualifiedName, List<IGlossaryTerm> terms)
            throws AtlanException {
     ...
     */
    @Deprecated
    public static Folder removeTerms(AtlanClient client, String qualifiedName, List<IGlossaryTerm> terms)
            throws AtlanException {
