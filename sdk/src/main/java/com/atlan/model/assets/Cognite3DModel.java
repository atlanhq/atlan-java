...
public class Cognite3DModel extends Asset
    ...
    /**
     * Retrieves a Cognite3DModel by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the Cognite3DModel to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the Cognite3DModel, including any relationships
     * @return the requested Cognite3DModel, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the Cognite3DModel does not exist or the provided GUID is not a Cognite3DModel
     */
    @JsonIgnore
     ...
     * @param id of the Cognite3DModel to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the Cognite3DModel, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the Cognite3DModel
     * @return the requested Cognite3DModel, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the Cognite3DModel does not exist or the provided GUID is not a Cognite3DModel
     */
    @JsonIgnore
    public static Cognite3DModel get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = Cognite3DModel.select(client)
                    .where(Cognite3DModel.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof Cognite3DModel) {
                return (Cognite3DModel) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
            Optional<Asset> asset = Cognite3DModel.select(client)
                    .where(Cognite3DModel.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .includeRelationshipAttributes(true)
                    .pageSize(1)
