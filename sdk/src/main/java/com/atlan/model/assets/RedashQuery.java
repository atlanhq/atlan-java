...
public class RedashQuery extends Asset implements IRedashQuery, IRedash, IBI, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a RedashQuery by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the RedashQuery to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the RedashQuery, including any relationships
     * @return the requested RedashQuery, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the RedashQuery does not exist or the provided GUID is not a RedashQuery
     */
    @JsonIgnore
     ...
     * @param id of the RedashQuery to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the RedashQuery, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the RedashQuery
     * @return the requested RedashQuery, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the RedashQuery does not exist or the provided GUID is not a RedashQuery
     */
    @JsonIgnore
    public static RedashQuery get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = RedashQuery.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof RedashQuery) {
