...
public class LookerQuery extends Asset implements ILookerQuery, ILooker, IBI, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a LookerQuery by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the LookerQuery to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the LookerQuery, including any relationships
     * @return the requested LookerQuery, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the LookerQuery does not exist or the provided GUID is not a LookerQuery
     */
    @JsonIgnore
     ...
     * @param id of the LookerQuery to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the LookerQuery, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the LookerQuery
     * @return the requested LookerQuery, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the LookerQuery does not exist or the provided GUID is not a LookerQuery
     */
    @JsonIgnore
    public static LookerQuery get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = LookerQuery.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof LookerQuery) {
