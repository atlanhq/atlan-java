...
public interface ICassandraTable {
    ...
    RelationField CASSANDRA_COLUMNS = new RelationField("cassandraColumns");

    /** Individual indexes contained within the table. */
    RelationField CASSANDRA_INDEXES = new RelationField("cassandraIndexes");

    /** Keyspace containing the table. */
    RelationField CASSANDRA_KEYSPACE = new RelationField("cassandraKeyspace");

    /** Bloom filter false positive chance for the CassandraTable. */
    NumericField CASSANDRA_TABLE_BLOOM_FILTER_FP_CHANCE =
            new NumericField("cassandraTableBloomFilterFPChance", "cassandraTableBloomFilterFPChance");

    /** CRC check chance for the CassandraTable. */
    NumericField CASSANDRA_TABLE_CRC_CHECK_CHANCE =
            new NumericField("cassandraTableCRCCheckChance", "cassandraTableCRCCheckChance");

    /** Caching behavior in Cassandra. */
    KeywordField CASSANDRA_TABLE_CACHING = new KeywordField("cassandraTableCaching", "cassandraTableCaching");

    /** Comment describing the CassandraTable's purpose or usage in Cassandra. */
    KeywordField CASSANDRA_TABLE_COMMENT = new KeywordField("cassandraTableComment", "cassandraTableComment");

    /** Compaction used for the CassandraTable in Cassandra. */
    KeywordField CASSANDRA_TABLE_COMPACTION = new KeywordField("cassandraTableCompaction", "cassandraTableCompaction");

    /** Compression used for the CassandraTable in Cassandra. */
    KeywordField CASSANDRA_TABLE_COMPRESSION =
            new KeywordField("cassandraTableCompression", "cassandraTableCompression");

    /** Local read repair chance in Cassandra. */
    NumericField CASSANDRA_TABLE_DC_LOCAL_READ_REPAIR_CHANCE =
            new NumericField("cassandraTableDCLocalReadRepairChance", "cassandraTableDCLocalReadRepairChance");

    /** Default time-to-live for the CassandraTable in Cassandra. */
    NumericField CASSANDRA_TABLE_DEFAULT_TTL = new NumericField("cassandraTableDefaultTTL", "cassandraTableDefaultTTL");

    /** Flags associated with the CassandraTable. */
    KeywordField CASSANDRA_TABLE_FLAGS = new KeywordField("cassandraTableFlags", "cassandraTableFlags");

    /** Grace period for garbage collection in the CassandraTable. */
    NumericField CASSANDRA_TABLE_GC_GRACE_SECONDS =
            new NumericField("cassandraTableGCGraceSeconds", "cassandraTableGCGraceSeconds");

    /** Unique identifier for the CassandraTable. */
    KeywordField CASSANDRA_TABLE_ID = new KeywordField("cassandraTableId", "cassandraTableId");

    /** Maximum index interval for the CassandraTable. */
    NumericField CASSANDRA_TABLE_MAX_INDEX_INTERVAL =
            new NumericField("cassandraTableMaxIndexInterval", "cassandraTableMaxIndexInterval");

    /** Memtable flush period for the CassandraTable (in milliseconds). */
    NumericField CASSANDRA_TABLE_MEMTABLE_FLUSH_PERIOD_IN_MS =
            new NumericField("cassandraTableMemtableFlushPeriodInMs", "cassandraTableMemtableFlushPeriodInMs");

    /** Minimum index interval for the CassandraTable. */
    NumericField CASSANDRA_TABLE_MIN_INDEX_INTERVAL =
            new NumericField("cassandraTableMinIndexInterval", "cassandraTableMinIndexInterval");

    /** Query used to create the CassandraTable in Cassandra. */
    KeywordField CASSANDRA_TABLE_QUERY = new KeywordField("cassandraTableQuery", "cassandraTableQuery");

    /** Read repair chance for the CassandraTable. */
    NumericField CASSANDRA_TABLE_READ_REPAIR_CHANCE =
            new NumericField("cassandraTableReadRepairChance", "cassandraTableReadRepairChance");

    /** Speculative retry setting for the CassandraTable. */
    KeywordField CASSANDRA_TABLE_SPECULATIVE_RETRY =
            new KeywordField("cassandraTableSpeculativeRetry", "cassandraTableSpeculativeRetry");

    /** Indicates whether the CassandraTable is virtual. */
    BooleanField CASSANDRA_TABLE_VIRTUAL = new BooleanField("cassandraTableVirtual", "cassandraTableVirtual");
    ...
    /** Indidivual columns contained in the table. */
    default SortedSet<ICassandraColumn> getCassandraColumns() {
        return null;
    }

    /** Individual indexes contained within the table. */
    default SortedSet<ICassandraIndex> getCassandraIndexes() {
        return null;
    }

    /** Keyspace containing the table. */
    default ICassandraKeyspace getCassandraKeyspace() {
        return null;
    }
    ...
    /** Bloom filter false positive chance for the CassandraTable. */
    Double getCassandraTableBloomFilterFPChance();

    /** CRC check chance for the CassandraTable. */
    Double getCassandraTableCRCCheckChance();

    /** Caching behavior in Cassandra. */
    Map<String, String> getCassandraTableCaching();

    /** Comment describing the CassandraTable's purpose or usage in Cassandra. */
    String getCassandraTableComment();

    /** Compaction used for the CassandraTable in Cassandra. */
    Map<String, String> getCassandraTableCompaction();

    /** Compression used for the CassandraTable in Cassandra. */
    Map<String, String> getCassandraTableCompression();

    /** Local read repair chance in Cassandra. */
    Double getCassandraTableDCLocalReadRepairChance();

    /** Default time-to-live for the CassandraTable in Cassandra. */
    Long getCassandraTableDefaultTTL();

    /** Flags associated with the CassandraTable. */
    SortedSet<String> getCassandraTableFlags();

    /** Grace period for garbage collection in the CassandraTable. */
    Long getCassandraTableGCGraceSeconds();

    /** Unique identifier for the CassandraTable. */
    String getCassandraTableId();

    /** Maximum index interval for the CassandraTable. */
    Long getCassandraTableMaxIndexInterval();

    /** Memtable flush period for the CassandraTable (in milliseconds). */
    Long getCassandraTableMemtableFlushPeriodInMs();

    /** Minimum index interval for the CassandraTable. */
    Long getCassandraTableMinIndexInterval();

    /** Name of the table for the Cassandra asset. */
    String getCassandraTableName();

    /** Unique name of table for Cassandra asset */
    String getCassandraTableQualifiedName();

    /** Query used to create the CassandraTable in Cassandra. */
    String getCassandraTableQuery();

    /** Read repair chance for the CassandraTable. */
    Double getCassandraTableReadRepairChance();

    /** Speculative retry setting for the CassandraTable. */
    String getCassandraTableSpeculativeRetry();

    /** Indicates whether the CassandraTable is virtual. */
    Boolean getCassandraTableVirtual();

    /** Name of view for Cassandra asset */
    String getCassandraViewName();
