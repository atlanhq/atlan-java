...
public class MetabaseDashboard extends Asset
     ...
     *
     * @return the minimal object necessary to relate to the MetabaseDashboard
     * @throws InvalidRequestException if any of the minimal set of required properties for a MetabaseDashboard relationship are not found in the initial object
     */
    @Override
    public MetabaseDashboard trimToReference() throws InvalidRequestException {
    ...
    /**
     * Start a fluent search that will return all MetabaseDashboard assets.
     ...
     * @return a fluent search that includes all MetabaseDashboard assets
     */
    ...
    /**
     * Reference to a MetabaseDashboard by GUID. Use this to create a relationship to this MetabaseDashboard,
     ...
     * @return reference to a MetabaseDashboard that can be used for defining a relationship to a MetabaseDashboard
     */
    public static MetabaseDashboard refByGuid(String guid, Reference.SaveSemantic semantic) {
        return MetabaseDashboard._internal().guid(guid).semantic(semantic).build();
    }

    /**
     * Reference to a MetabaseDashboard by qualifiedName. Use this to create a relationship to this MetabaseDashboard,
     * where the relationship should be replaced.
     *
     ...
     * @return reference to a MetabaseDashboard that can be used for defining a relationship to a MetabaseDashboard
     */
    public static MetabaseDashboard refByQualifiedName(String qualifiedName, Reference.SaveSemantic semantic) {
    ...
    /**
     * Retrieves a MetabaseDashboard by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the MetabaseDashboard to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MetabaseDashboard, including any relationships
     * @return the requested MetabaseDashboard, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MetabaseDashboard does not exist or the provided GUID is not a MetabaseDashboard
     */
    @JsonIgnore
     ...
     * @param id of the MetabaseDashboard to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MetabaseDashboard, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the MetabaseDashboard
     * @return the requested MetabaseDashboard, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MetabaseDashboard does not exist or the provided GUID is not a MetabaseDashboard
     */
    @JsonIgnore
    public static MetabaseDashboard get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = MetabaseDashboard.select(client)
                    .where(MetabaseDashboard.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof MetabaseDashboard) {
                return (MetabaseDashboard) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
            Optional<Asset> asset = MetabaseDashboard.select(client)
                    .where(MetabaseDashboard.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .includeRelationshipAttributes(true)
                    .pageSize(1)
