...
public class MatillionComponent extends Asset
    ...
    /**
     * Retrieves a MatillionComponent by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the MatillionComponent to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MatillionComponent, including any relationships
     * @return the requested MatillionComponent, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MatillionComponent does not exist or the provided GUID is not a MatillionComponent
     */
    @JsonIgnore
     ...
     * @param id of the MatillionComponent to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the MatillionComponent, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the MatillionComponent
     * @return the requested MatillionComponent, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the MatillionComponent does not exist or the provided GUID is not a MatillionComponent
     */
    @JsonIgnore
    public static MatillionComponent get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = MatillionComponent.select(client)
                    .where(MatillionComponent.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof MatillionComponent) {
            ...
            Optional<Asset> asset = MatillionComponent.select(client)
                    .where(MatillionComponent.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .includeRelationshipAttributes(true)
                    .pageSize(1)
    ...
    /**
     * Builds the minimal object necessary to apply an update to a MatillionComponent,
     * from a potentially more-complete MatillionComponent object.
     *
     * @return the minimal object necessary to update the MatillionComponent, as a builder
     * @throws InvalidRequestException if any of the minimal set of required fields for a MatillionComponent are not present in the initial object
     */
    @Override
