...
public class CogniteAsset extends Asset implements ICogniteAsset, ICognite, ISaaS, ICatalog, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a CogniteAsset by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the CogniteAsset to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the CogniteAsset, including any relationships
     * @return the requested CogniteAsset, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the CogniteAsset does not exist or the provided GUID is not a CogniteAsset
     */
    @JsonIgnore
     ...
     * @param id of the CogniteAsset to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the CogniteAsset, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the CogniteAsset
     * @return the requested CogniteAsset, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the CogniteAsset does not exist or the provided GUID is not a CogniteAsset
     */
    @JsonIgnore
    public static CogniteAsset get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = CogniteAsset.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof CogniteAsset) {
                return (CogniteAsset) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
    ...
    public abstract static class CogniteAssetBuilder<C extends CogniteAsset, B extends CogniteAssetBuilder<C, B>>
