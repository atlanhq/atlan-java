...
public class S3Bucket extends Asset implements IS3Bucket, IS3, IObjectStore, IAWS, ICatalog, IAsset, IReferenceable, ICloud {
     ...
     */
    @JsonIgnore
    public static S3Bucket get(AtlanClient client, String id, boolean includeAllRelationships) throws AtlanException {
        if (id == null) {
    ...
    /**
     * Retrieves a S3Bucket by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the S3Bucket to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the S3Bucket, including any relationships
     * @return the requested S3Bucket, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the S3Bucket does not exist or the provided GUID is not a S3Bucket
     */
    @JsonIgnore
    public static S3Bucket get(AtlanClient client, String id, Collection<AtlanField> attributes) throws AtlanException {
     ...
     * @param id of the S3Bucket to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the S3Bucket, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the S3Bucket
     * @return the requested S3Bucket, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the S3Bucket does not exist or the provided GUID is not a S3Bucket
     */
    @JsonIgnore
    public static S3Bucket get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = S3Bucket.select(client)
                    .where(S3Bucket.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof S3Bucket) {
                return (S3Bucket) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
            Optional<Asset> asset = S3Bucket.select(client)
                    .where(S3Bucket.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .includeRelationshipAttributes(true)
                    .pageSize(1)
...
/**
    ...
    /**
     * Builds the minimal object necessary to apply an update to a S3Bucket,
     * from a potentially more-complete S3Bucket object.
     *
     * @return the minimal object necessary to update the S3Bucket, as a builder
     * @throws InvalidRequestException if any of the minimal set of required fields for a S3Bucket are not present in the initial object
     */
    @Override
