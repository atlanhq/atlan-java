...
public class SigmaDataElementField extends Asset
    ...
    /**
     * Builds the minimal object necessary to create a relationship to a SigmaDataElementField, from a potentially
     * more-complete SigmaDataElementField object.
     *
     * @return the minimal object necessary to relate to the SigmaDataElementField
     * @throws InvalidRequestException if any of the minimal set of required properties for a SigmaDataElementField relationship are not found in the initial object
     */
    @Override
    ...
    /**
     * Start a fluent search that will return all SigmaDataElementField assets.
     ...
     * @return a fluent search that includes all SigmaDataElementField assets
     */
    ...
    /**
     * Reference to a SigmaDataElementField by GUID. Use this to create a relationship to this SigmaDataElementField,
    ...
    public static SigmaDataElementField refByGuid(String guid, Reference.SaveSemantic semantic) {
        return SigmaDataElementField._internal().guid(guid).semantic(semantic).build();
    }

    /**
     * Reference to a SigmaDataElementField by qualifiedName. Use this to create a relationship to this SigmaDataElementField,
     * where the relationship should be replaced.
     *
    ...
    /**
     * Retrieves a SigmaDataElementField by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the SigmaDataElementField to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SigmaDataElementField, including any relationships
     * @return the requested SigmaDataElementField, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SigmaDataElementField does not exist or the provided GUID is not a SigmaDataElementField
     */
    @JsonIgnore
     ...
     * @param id of the SigmaDataElementField to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the SigmaDataElementField, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the SigmaDataElementField
     * @return the requested SigmaDataElementField, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the SigmaDataElementField does not exist or the provided GUID is not a SigmaDataElementField
     */
    @JsonIgnore
    public static SigmaDataElementField get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = SigmaDataElementField.select(client)
                    .where(SigmaDataElementField.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof SigmaDataElementField) {
                return (SigmaDataElementField) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
            Optional<Asset> asset = SigmaDataElementField.select(client)
                    .where(SigmaDataElementField.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .pageSize(1)
                    .stream()
