...
public class QlikApp extends Asset implements IQlikApp, IQlik, IBI, ICatalog, IAsset, IReferenceable {
     ...
     */
    @JsonIgnore
    public static QlikApp get(AtlanClient client, String id, boolean includeAllRelationships) throws AtlanException {
        if (id == null) {
    ...
    /**
     * Retrieves a QlikApp by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the QlikApp to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the QlikApp, including any relationships
     * @return the requested QlikApp, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the QlikApp does not exist or the provided GUID is not a QlikApp
     */
    @JsonIgnore
    public static QlikApp get(AtlanClient client, String id, Collection<AtlanField> attributes) throws AtlanException {
     ...
     * @param id of the QlikApp to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the QlikApp, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the QlikApp
     * @return the requested QlikApp, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the QlikApp does not exist or the provided GUID is not a QlikApp
     */
    @JsonIgnore
    public static QlikApp get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = QlikApp.select(client)
                    .where(QlikApp.GUID.eq(id))
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof QlikApp) {
                return (QlikApp) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
            Optional<Asset> asset = QlikApp.select(client)
                    .where(QlikApp.QUALIFIED_NAME.eq(id))
                    .includesOnResults(attributes)
                    .includesOnRelations(attributesOnRelated)
                    .pageSize(1)
                    .stream()
