...
public class TagAttachment extends Asset implements ITagAttachment, IAsset, IReferenceable {
    ...
    /**
     * Retrieves a TagAttachment by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the TagAttachment to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the TagAttachment, including any relationships
     * @return the requested TagAttachment, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the TagAttachment does not exist or the provided GUID is not a TagAttachment
     */
    @JsonIgnore
     ...
     * @param id of the TagAttachment to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the TagAttachment, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the TagAttachment
     * @return the requested TagAttachment, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the TagAttachment does not exist or the provided GUID is not a TagAttachment
     */
    @JsonIgnore
    public static TagAttachment get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = TagAttachment.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof TagAttachment) {
                return (TagAttachment) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
