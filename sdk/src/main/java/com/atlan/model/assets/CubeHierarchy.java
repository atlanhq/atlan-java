...
public class CubeHierarchy extends Asset
    ...
    /**
     * Retrieves a CubeHierarchy by one of its identifiers, with only the requested attributes (and relationships).
     *
     * @param client connectivity to the Atlan tenant from which to retrieve the asset
     * @param id of the CubeHierarchy to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the CubeHierarchy, including any relationships
     * @return the requested CubeHierarchy, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the CubeHierarchy does not exist or the provided GUID is not a CubeHierarchy
     */
    @JsonIgnore
     ...
     * @param id of the CubeHierarchy to retrieve, either its GUID or its full qualifiedName
     * @param attributes to retrieve for the CubeHierarchy, including any relationships
     * @param attributesOnRelated to retrieve on each relationship retrieved for the CubeHierarchy
     * @return the requested CubeHierarchy, with only its minimal information and the requested attributes (and relationships)
     * @throws AtlanException on any error during the API invocation, such as the {@link NotFoundException} if the CubeHierarchy does not exist or the provided GUID is not a CubeHierarchy
     */
    @JsonIgnore
    public static CubeHierarchy get(
            AtlanClient client,
            String id,
        ...
        if (id == null) {
            ...
            Optional<Asset> asset = CubeHierarchy.select(client)
            ...
            if (!asset.isPresent()) {
                throw new NotFoundException(ErrorCode.ASSET_NOT_FOUND_BY_GUID, id);
            } else if (asset.get() instanceof CubeHierarchy) {
                return (CubeHierarchy) asset.get();
            } else {
                throw new NotFoundException(ErrorCode.ASSET_NOT_TYPE_REQUESTED, id, TYPE_NAME);
            }
        } else {
