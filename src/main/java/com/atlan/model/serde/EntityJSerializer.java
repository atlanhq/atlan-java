...
@Slf4j
public class EntityJSerializer extends StdSerializer<EntityJ> {
    private static final long serialVersionUID = 2L;

    public EntityJSerializer() {
        this(null);
    }

    public EntityJSerializer(Class<EntityJ> t) {
        super(t);
    }

    @Override
    public void serializeWithType(
            EntityJ value, JsonGenerator gen, SerializerProvider serializers, TypeSerializer typeSer)
            throws IOException {
        serialize(value, gen, serializers);
    }

    @Override
    public void serialize(EntityJ entity, JsonGenerator gen, SerializerProvider sp)
                    ...
                    if (field.isAnnotationPresent(Attribute.class)) {
                        ...
                        if (nullFields.contains(fieldName)) {
                            ...
                            if (field.getType() == List.class || field.getType() == Set.class) {
                                attrValue = RemovableJ.EMPTY_LIST;
                            } else {
                                attrValue = RemovableJ.NULL;
                            }
                        } else {
                        ...
                        Map<String, CustomMetadataAttributesJ> cm = entity.getCustomMetadataSets();
                        if (cm != null) {
                            CustomMetadataCacheJ.getBusinessAttributesFromCustomMetadataJ(cm, businessAttributes);
                        }
