...
public abstract class AssetJ extends EntityJ {
    ...
    protected abstract AssetJBuilder<?, ?> trimToRequired();
     ...
     */
    protected static EntityJ updateCertificate(
            AssetJ.AssetJBuilder<?, ?> builder, AtlanCertificateStatus certificate, String message)
     ...
     * @throws AtlanException on any API problems
     */
    protected static EntityJ removeCertificate(AssetJ.AssetJBuilder<?, ?> builder) throws AtlanException {
        AssetJ asset = builder.build();
        asset.removeCertificate();
        EntityMutationResponseJ response = asset.upsert();
     ...
     * @throws AtlanException on any API problems
     */
    protected static EntityJ updateAnnouncement(
            AssetJ.AssetJBuilder<?, ?> builder, AtlanAnnouncementType type, String title, String message)
            throws AtlanException {
     ...
     * @throws AtlanException on any API problems
     */
    protected static EntityJ removeAnnouncement(AssetJ.AssetJBuilder<?, ?> builder) throws AtlanException {
        AssetJ asset = builder.build();
    ...
    private static EntityJ updateAttributes(AssetJ asset) throws AtlanException {
        EntityMutationResponseJ response = EntityBulkEndpointJ.upsert(asset, false, false);
        if (response != null && !response.getUpdatedEntities().isEmpty()) {
            return response.getUpdatedEntities().get(0);
        }
        return null;
    ...
    private static EntityJ updateAttributes(String typeName, String qualifiedName, AssetJ asset) throws AtlanException {
        EntityMutationResponseJ response =
                EntityUniqueAttributesEndpointJ.updateAttributes(typeName, qualifiedName, asset);
     ...
     * @throws AtlanException on any API problems
     */
    protected static EntityJ replaceTerms(AssetJ.AssetJBuilder<?, ?> builder, List<ReferenceJ> terms)
            throws AtlanException {
     ...
     * @throws AtlanException on any API problems
     */
    protected static EntityJ appendTerms(String typeName, String qualifiedName, List<ReferenceJ> terms)
            throws AtlanException {
        AssetJ existing = getExistingAsset(typeName, qualifiedName);
        if (existing != null) {
            Set<ReferenceJ> replacementTerms = new LinkedHashSet<>();
            Set<ReferenceJ> existingTerms = existing.getMeanings();
            if (existingTerms != null) {
                for (ReferenceJ term : existingTerms) {
            ...
            AssetJ.AssetJBuilder<?, ?> minimal = existing.trimToRequired();
     ...
     * @throws AtlanException on any API problems
     */
    protected static EntityJ removeTerms(String typeName, String qualifiedName, List<GuidReferenceJ> terms)
            throws AtlanException {
        AssetJ existing = getExistingAsset(typeName, qualifiedName);
        if (existing != null) {
            Set<ReferenceJ> replacementTerms = new LinkedHashSet<>();
            Set<ReferenceJ> existingTerms = existing.getMeanings();
            Set<String> removeGuids = terms.stream().map(ReferenceJ::getGuid).collect(Collectors.toSet());
            for (ReferenceJ term : existingTerms) {
    ...
    private static AssetJ getExistingAsset(String typeName, String qualifiedName) throws AtlanException {
        EntityResponseJ response = EntityUniqueAttributesEndpointJ.retrieve(typeName, qualifiedName, false, false);
        if (response != null) {
            return (AssetJ) response.getEntity();
        }
        return null;
    }

    private static EntityJ updateRelationships(AssetJ asset) throws AtlanException {
        String typeNameToUpdate = asset.getTypeName();
        EntityMutationResponseJ response = EntityBulkEndpointJ.upsert(asset, false, false);
        if (response != null && !response.getUpdatedEntities().isEmpty()) {
            for (EntityJ result : response.getUpdatedEntities()) {
