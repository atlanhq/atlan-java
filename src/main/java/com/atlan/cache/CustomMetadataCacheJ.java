...
import com.atlan.model.serde.RemovableJ;
 ...
 */
@Slf4j
public class CustomMetadataCacheJ {

    private static Map<String, CustomMetadataDefJ> cacheById = new ConcurrentHashMap<>();
    ...
    private static synchronized void refreshCache() throws AtlanException {
        ...
        TypeDefResponseJ response = TypeDefsEndpointJ.getTypeDefs(AtlanTypeCategory.BUSINESS_METADATA);
        List<CustomMetadataDefJ> customMetadata;
        if (response != null) {
        ...
        for (CustomMetadataDefJ bmDef : customMetadata) {
            ...
            for (AttributeDefJ attributeDef : bmDef.getAttributeDefs()) {
    ...
    public static Map<String, Object> getEmptyAttributes(String customMetadataName) throws AtlanException {
        ...
        for (String attrName : attributes.keySet()) {
            empty.put(attrName, RemovableJ.NULL);
        }
        return empty;
    }
    ...
    public static void getBusinessAttributesFromCustomMetadataJ(
            Map<String, CustomMetadataAttributesJ> customMetadata, Map<String, Map<String, Object>> businessAttributes)
            throws AtlanException {
        if (customMetadata != null) {
            for (Map.Entry<String, CustomMetadataAttributesJ> entry : customMetadata.entrySet()) {
                String cmName = entry.getKey();
                String cmId = getIdForName(cmName);
                CustomMetadataAttributesJ attrs = entry.getValue();
     ...
     * @throws AtlanException on any API communication problem if the cache needs to be refreshed
     */
    public static Map<String, CustomMetadataAttributesJ> getCustomMetadataFromBusinessAttributes(
            JsonNode businessAttributes) throws AtlanException {
        Map<String, CustomMetadataAttributesJ> map = new LinkedHashMap<>();
        ...
        while (itrCM.hasNext()) {
            ...
            CustomMetadataAttributesJ.CustomMetadataAttributesJBuilder<?, ?> builder =
                    CustomMetadataAttributesJ.builder();
            ...
            CustomMetadataAttributesJ cma = builder.build();
            if (!cma.isEmpty()) {
